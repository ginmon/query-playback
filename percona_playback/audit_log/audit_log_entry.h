#ifndef PERCONA_PLAYBACK_AUDIT_LOG_ENTRY_H
#define PERCONA_PLAYBACK_AUDIT_LOG_ENTRY_H
...
class AuditLogEntry : public QueryEntry
{
    private:
      uint64_t thread_id;
      uint64_t rows_sent;
    ...
    public:
      ...
      AuditLogEntry() : thread_id(0), rows_sent(0), rows_examined(0), query_time(0) {}

      virtual uint64_t getThreadId() const { return thread_id; }

      inline double getQueryTime() { return query_time; }

      void add_query_line(const std::string &s, const std::string &schema_name);
...
#endif /* PERCONA_PLAYBACK_AUDIT_LOG_ENTRY_H */
